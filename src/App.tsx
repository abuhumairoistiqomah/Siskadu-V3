...
export default function App() {
  ...
  const [loggedInUser, setLoggedInUser] = useState<string>(() => {
    const savedName = localStorage.getItem('siskadu_username');
    if (!savedName && localStorage.getItem('siskadu_logged_in') === 'true') {
      return 'Admin Utama';
    }
    return savedName || '';
  });
  ...
  // Create the filtered dataset for display purposes (especially if "dirpen" user is logged in)
  const displayedComplaints = useMemo(() => {
    const isDirpen = loggedInUser.toLowerCase().includes('dirpen');
    if (isLoggedIn && isDirpen) {
      return complaints.filter(c => c.pic && c.pic.toLowerCase().includes('dirpen'));
    }
    return complaints;
  }, [complaints, isLoggedIn, loggedInUser]);
...
function doPost(e) {
  ...
  const uniqueTemas = useMemo(() => {
    const list = displayedComplaints.map(c => c.tema);
    return ['Semua', ...Array.from(new Set(list))];
  }, [displayedComplaints]);

  // Filter unique statuses
  const uniqueStatuses = useMemo(() => {
    const list = displayedComplaints.map(c => c.status);
    return ['Semua', ...Array.from(new Set(list))];
  }, [displayedComplaints]);

  // Dynamic lists for advanced filters
  const uniqueDates = useMemo(() => {
    const list = displayedComplaints.map(c => c.tanggalDiterima).filter(Boolean);
    return ['Semua', ...Array.from(new Set(list))];
  }, [displayedComplaints]);

  const uniqueMedias = useMemo(() => {
    const list = displayedComplaints.map(c => c.media).filter(Boolean);
    return ['Semua', ...Array.from(new Set(list))];
  }, [displayedComplaints]);

  const uniqueClasses = useMemo(() => {
    const list = displayedComplaints.map(c => c.kelas).filter(Boolean);
    return ['Semua', ...Array.from(new Set(list))];
  }, [displayedComplaints]);

  const uniqueTargets = useMemo(() => {
    const list = displayedComplaints.map(c => c.target).filter(Boolean);
    return ['Semua', ...Array.from(new Set(list))];
  }, [displayedComplaints]);

  const uniquePics = useMemo(() => {
    const list = displayedComplaints.map(c => c.pic).filter(Boolean);
    return ['Semua', ...Array.from(new Set(list))];
  }, [displayedComplaints]);

  const uniqueFinishDates = useMemo(() => {
    const list = displayedComplaints.map(c => c.tanggalSelesai).filter(Boolean);
    return ['Semua', ...Array.from(new Set(list))];
  }, [displayedComplaints]);

  // Filter complaints based on Search query & Dropdown selections (covers 14 columns)
  const filteredComplaints = useMemo(() => {
    return displayedComplaints.filter(c => {
    ...
    displayedComplaints, searchQuery, filterTema, filterStatus, filterUrgensi,
  ...
  const metrics = useMemo(() => {
    const total = displayedComplaints.length;
    const onProgress = displayedComplaints.filter(c => {
    ...
    const resolved = displayedComplaints.filter(c => {
      const st = (c.status || '').toLowerCase();
      return st === 'selesai' || st === 'solved' || st === 'resolved';
    }).length;

    const highUrgency = displayedComplaints.filter(c => {
  ...
  const categoryStats = useMemo(() => {
    ...
    const total = displayedComplaints.length || 1;
      ...
      try {
        ...
        if (result && result.status === 'success' && result.type === 'accounts' && Array.isArray(result.data)) {
          ...
          if (match) {
            ...
            localStorage.setItem('siskadu_username', match.id);
            setLoginError('');
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('siskadu_username');
    setLoginId('');
    setLoginPassword('');
  };
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-2.5 select-none">
                <input
                  ...
                  className="w-4 h-4 rounded border-slate-600 text-blue-600 focus:ring-blue-500/20 focus:ring-offset-slate-800 bg-slate-700/50 cursor-pointer"
                />
                <label htmlFor="show-password" className="text-xs text-slate-400 font-medium cursor-pointer">
                  Tampilkan Password
                </label>
              </div>
            </div>
      ...
      <div id="sub-header-nav" className="bg-slate-800 text-white flex items-center px-4 md:px-6 shrink-0 h-11 border-b border-slate-700 justify-between z-20 overflow-visible">
        ...
        <div className="flex md:hidden items-center justify-between gap-3 w-full">
              ...
              <>
                ...
                <div className="absolute top-[100%] left-0 right-0 mt-1 bg-slate-900 border border-slate-700 rounded-lg shadow-xl py-1 z-50 animate-in fade-in slide-in-from-top-1 duration-150 max-h-80 overflow-y-auto">
                  ...
                  >
                    ...
                    <span>Historis Pengaduan ({displayedComplaints.length})</span>
                  </button>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {/* Elegant Mobile User Badge */}
            <div className="flex items-center gap-1.5 bg-slate-700/60 border border-slate-600 rounded-full pl-2 pr-3 py-1 text-[11px] text-slate-200 font-bold max-w-[125px] truncate" title={`Akun Aktif: ${loggedInUser || 'Admin Utama'}`}>
              <div className="w-5 h-5 rounded-full bg-indigo-600 flex items-center justify-center text-white text-[9px] font-black border border-indigo-400 shrink-0">
                {(loggedInUser || 'A').charAt(0).toUpperCase()}
              </div>
              <span className="truncate leading-none shrink">{loggedInUser || 'Admin Utama'}</span>
            </div>

            <button
              onClick={handleLogout}
              className="flex items-center justify-center p-2 bg-red-600/20 hover:bg-red-600/30 text-red-300 hover:text-red-200 text-xs font-bold rounded-full transition-colors border border-red-500/20 cursor-pointer shrink-0"
        ...
        <div className="hidden md:flex gap-1 h-full items-end min-w-max">
          ...
          >
            ...
            Historis Pengaduan & Filter ({displayedComplaints.length})
          </button>
         ...
         <div className="hidden md:flex items-center justify-end gap-4">
           ...
           {/* Elegant Desktop User Badge */}
           <div className="flex items-center gap-2.5 bg-slate-700/50 border border-slate-600 rounded-full pl-2 pr-4 py-1 text-xs text-indigo-200 font-bold shadow-sm select-none shrink-0 hover:border-slate-500 hover:bg-slate-700/70 transition-all">
             <div className="w-5.5 h-5.5 rounded-full bg-gradient-to-tr from-indigo-600 to-violet-500 flex items-center justify-center text-white text-[10px] font-black border border-indigo-400 shrink-0">
               {(loggedInUser || 'A').charAt(0).toUpperCase()}
             </div>
             <div className="flex flex-col text-left leading-tight shrink">
               <span className="text-[9px] text-slate-400 font-normal">Sesi Aktif</span>
               <span className="text-[11px] text-slate-100 font-semibold truncate max-w-[120px]" title={loggedInUser || 'Admin Utama'}>
                 {loggedInUser || 'Admin Utama'}
               </span>
             </div>
             {loggedInUser.toLowerCase().includes('dirpen') && (
               <span className="ml-0.5 bg-amber-500/20 text-amber-300 text-[9px] px-1.5 py-0.5 rounded border border-amber-500/30 font-extrabold uppercase tracking-wider shrink-0 animate-pulse">
                 Dirpen
               </span>
             )}
           </div>
           ...
           <button
             onClick={handleLogout}
             className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-red-600/20 hover:bg-red-600/30 text-red-300 hover:text-red-200 text-[11px] font-bold rounded-md transition-colors border border-red-500/20 cursor-pointer shrink-0"
             title="Keluar dari Sistem"
           >
             <LogOut className="w-3.5 h-3.5" />
             <span>Keluar</span>
        ...
        {activeTab === 'dashboard' ? (
                    ...
                    {complaints.length === 0 ? (
                      ...
                      defaultSortComplaints(complaints).slice(0, 5).map((comp, idx) => (
                        <div
                          key={idx}
                          onClick={() => handleSelectComplaint(comp)}
                          className="p-3.5 hover:bg-slate-50 active:bg-slate-100 transition-all cursor-pointer flex flex-col gap-2.5 bg-white rounded-xl border border-slate-200 shadow-3xs"
                        >
                          <div className="flex items-center justify-between gap-2 border-b border-slate-100 pb-2">
                            <span className="font-bold text-slate-700 text-xs">{formatToIndonesianDate(comp.tanggalDiterima)}</span>
                            <div className="flex items-center gap-1.5 shrink-0">
                              <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold border ${getUrgencyStyles(comp.urgensi)}`}>
                                {comp.urgensi}
                              </span>
                              <span className="flex items-center gap-1">
                                <span className={`w-1.5 h-1.5 rounded-full ${getStatusDot(comp.status)}`}></span>
                                <span className="font-extrabold text-slate-700 text-[10px] uppercase">{comp.status}</span>
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-2 text-[11px]">
                            <div>
                              <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">PELAPOR</p>
                              <p className="font-semibold text-slate-800 truncate">{comp.namaPelapor}</p>
                            </div>
                            <div>
                              <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">SISWA (KELAS)</p>
                              <p className="font-semibold text-slate-800 truncate">
                                {comp.namaSiswa} <span className="text-slate-500 font-bold">({comp.kelas})</span>
                              </p>
                            </div>
                          </div>

                          <div className="text-[11px] bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">TEMA & TARGET</p>
                            <p className="font-bold text-slate-700 truncate">{comp.tema}</p>
                            <p className="text-[10px] text-slate-500 truncate" title={comp.namaTarget || comp.target}>
                              Target: <span className="font-semibold text-slate-600">{cleanTargetName(comp.namaTarget || comp.target)}</span>
                            </p>
                          </div>
                        </div>
                      ))
          ...
          <div className="flex-1 bg-white rounded-lg border border-slate-200 shadow-xs flex flex-col min-h-0 overflow-hidden">
            ...
            <div className="flex-1 overflow-auto">
              ...
              {/* Mobile Cards List View */}
              <div className="block lg:hidden p-3 space-y-3.5 bg-slate-50">
                {filteredAndSortedComplaints.length === 0 ? (
                  <div className="flex flex-col items-center justify-center gap-2 py-12 text-slate-400 bg-white rounded-xl border border-slate-200 shadow-3xs">
                    <AlertCircle className="w-8 h-8 text-slate-300" />
                    <p className="font-semibold text-slate-500 text-xs">Tidak ada data pengaduan ditemukan</p>
                    <p className="text-[10px] text-slate-400">Ubah filter pencarian atau sinkronisasi ulang.</p>
                  </div>
                  ...
                  filteredAndSortedComplaints.map((comp, idx) => (
                    <div
                      key={idx}
                      onClick={() => handleSelectComplaint(comp)}
                      className="p-4 bg-white rounded-xl border border-slate-200 hover:border-blue-300 active:bg-slate-50 transition-all cursor-pointer flex flex-col gap-3 shadow-3xs"
                    >
                      {/* Card Header */}
                      <div className="flex items-center justify-between gap-2 border-b border-slate-100 pb-2">
                        <div className="flex items-center gap-2">
                          <span className="px-1.5 py-0.5 rounded bg-slate-100 text-slate-600 font-bold text-[9px] uppercase">
                            {comp.media}
                          </span>
                          <span className="font-bold text-slate-700 text-xs">{formatToIndonesianDate(comp.tanggalDiterima)}</span>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 text-[11px]">
                        <div>
                          <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">PELAPOR</p>
                          <p className="font-semibold text-slate-800">{comp.namaPelapor}</p>
                          <p className="text-[9px] text-slate-400">Orang Tua / Wali</p>
                        </div>
                        <div>
                          <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">SISWA (KELAS)</p>
                          <p className="font-semibold text-slate-800">{comp.namaSiswa}</p>
                          <p className="text-[10px] text-slate-500 font-bold">Kelas: {comp.kelas}</p>
                        </div>
                      </div>
                      ...
                      <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-100 text-[11px]">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">TEMA & TARGET</p>
                            <p className="font-bold text-slate-700">{comp.tema}</p>
                          ...
                          {comp.pic && comp.pic !== '-' && (
                            <div className="text-right shrink-0">
                              <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">PIC</p>
                              <span className="bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded text-[10px] font-bold border border-indigo-100 block mt-0.5">
                                {comp.pic}
                              </span>
                            </div>
                          )}
                      ...
                      {/* Isi Laporan */}
                      <div className="text-xs text-slate-600 line-clamp-3 bg-blue-50/20 p-2.5 rounded-lg border border-blue-500/5">
                        <p className="text-[9px] text-blue-500/80 font-extrabold uppercase tracking-wider mb-1">Rincian Laporan</p>
                        <p className="leading-normal">{comp.isi}</p>
                      </div>

                      {/* Tindakan / Realisasi */}
                      {comp.tindakan && comp.tindakan !== '-' && (
                        <div className="text-xs text-slate-600 bg-emerald-50/25 p-2.5 rounded-lg border border-emerald-500/10 italic">
                          <p className="text-[9px] text-emerald-600 font-extrabold uppercase tracking-wider not-italic mb-1">Tindakan / Realisasi</p>
                          <p className="leading-normal">"{comp.tindakan}"</p>
                        </div>
                      )}
                    </div>
                  ))
